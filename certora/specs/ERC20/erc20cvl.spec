...
methods {
    ...
    // function _.safeApprove(address token, address a, uint256 x)                    external with (env e) => approveCVL(token, e.msg.sender, a, x) expect bool;
    function _.safeTransfer(address token, address a, uint256 x)                   internal with (env e) => transferCVL(token, e.msg.sender, a, x) expect bool;
    function _.safeTransferFrom(address token, address a, address b, uint256 x)    internal with (env e) => transferFromCVL(token, e.msg.sender, a, b, x) expect bool;
    // function _.safeApprove(address token, address a, uint256 x)                    external => NONDET;
    // function _.safeTransfer(address token, address a, uint256 x)                   external  => NONDET;
    // function _.safeTransferFrom(address token, address a, address b, uint256 x)    external  => NONDET;
